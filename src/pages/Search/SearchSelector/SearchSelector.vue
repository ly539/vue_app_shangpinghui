  ...
  export default {
    ...
    methods:{
      ...
      tradeMarkHandler(trademark){
        // 点击品牌,还需要整理参数,向服务器发送请求获取相应的数据进行展示
        // 在哪里发请求? 父组件发请求,因为父组件中searchParams参数带给服务器参数,子组件把你点击的品牌信息,需要给父组件传递过去
        this.$emit('trademarkInfo', trademark)
      },
      // 平台售卖属性, 当触发这个函数的时候,就会同时触发attrInfo事件,并且将attr, attrValue值传过去
      attrInfo(attr, attrValue){
        this.$emit("attrInfo", attr, attrValue)
      }
    },
    computed:{
      ...mapGetters(['attrsLists','trademarkLists']),
      trademarkList(){
        return this.trademarkLists.trademarkList
      },
      attrsList(){
        return this.attrsLists.attrsList
      }
    }
  }
</script>
