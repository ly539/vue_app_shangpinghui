        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 路由跳转之前还发送请求, 把购买的产品信息通过请求的形式通知服务器, 服务器将进行相应存储 -->
...
<script>
  ...
  export default {
    ...
    computed:{
      ...
      skuImageList(){
        // 如果服务器数据没有回来,skuInfo这个对象是空对象,那摩就会出现undefined
        return this.skuInfo.skuImageList||[]
      },
    },
    methods:{
      // 产品属性值切换
      changeActive(saleAttrList, arr){
        // 遍历全部售卖的属性值ischecked为0表示没有高亮了
        arr.forEach(item => {
          item.isChecked = '0'
        });
        saleAttrList.isChecked = 1
      },
      // 表单元素修改产品个数
      changeSkuNum(event){
      ...
      // 加入购物车的回调函数
      async addShopcar(){
        // 1. 发请求 ---将产品加入到数据库, 调用addOrdateShopCart, 返回一个promise
        // 2. 服务器存储成功 ---进行路由跳转
        // 3. 失败,汇给用户进行提示
        try{
          // 成功
          ...
          // 进行路由跳转, 需要将产品信息带给下一级的路由组件
          // 产品信息的数据比较复杂skuInfo,通过会话存储, 一般用字符串进行存储,所以需要将对象改为字符串
