      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="(cart, index) in cartList" :key="cart.id">
          ...
          <li class="cart-list-con5">
            ...
            <input autocomplete="off" type="text" minnum="1" class="itxt" :value = "cart.skuNum" :change = "handler(all, $event.target.value, cart)">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 修改某一个产品的个数
    // 当点击次数过多, 服务器更新不及时,就会出现错误, 所以采用节流(单位时间内只执行一次)的方式 
      handler : throttle(async function(type, disNum, cart){
        switch(type){
          case "plus":
            ...
            break;
          case "minus":
            // 判断产品个数 >1,才可以传递给服务器-1
            // 如果产品个数小于等于1, 传递给服务器个数0
            if(cart.skuNum > 1){
              disNum = -1;
            }else{
              // 产品个数小于等于1
            ...
            break;
          case "change":
            // 用户输入进来的最终量是非法的(带有汉字)或者出现负数, 带给服务器数字0
      ...
      updateCheckedById(cart, event){
        try{
          ...
          this.$store.dispatch('updateCheckedById', {skuId: cart.id, isChecked: isChecked});
          this.getDate()
        }catch(error){
          // 如果失败,就提示
          alert(error.message)
